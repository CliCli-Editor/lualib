...
---@field on_hit? fun(self: Mover, unit: Unit) # Collision unit callback (Special effects do not support collision units)
---@field on_block? fun(self: Mover) # Collision terrain callback (Special effects do not support collision terrain)
...
--The cpp interface of the todo engine has not been updated for a long time. It is time for a update...
...
function M.mover_line(mover_unit, mover_data)
    ...
    if clicli.config.mover.enable_internal_regist and mover_unit.type ~= 'particle' then
